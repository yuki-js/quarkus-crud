...
public class OpenApiContractTest {
  ...
  @Test
  @Order(3)
  public void testGetAllEventsContract() {
    // GET /api/events should conform to OpenAPI spec
    given().filter(validationFilter).when().get("/api/events").then().statusCode(200);
  }
  ...
  public void testCreateEventContract() {
    ...
    var response =
        given()
            ...
            .header("Authorization", "Bearer " + jwtToken)
            .contentType(ContentType.JSON)
            .body("{\"name\":\"Contract Test Event\",\"description\":\"Testing OpenAPI contract\"}")
            .when()
            .post("/api/events")
            .then()
            .statusCode(201)
    ...
    Assertions.assertNotNull(testEventId, "Created event should have an ID");
  }

  @Test
  @Order(5)
  public void testGetEventByIdContract() {
    // GET /api/events/{id} should conform to OpenAPI spec
    given()
        .filter(validationFilter)
        .when()
  ...
  @Test
  @Order(6)
  public void testGetMyEventsContract() {
    // GET /api/events/my should conform to OpenAPI spec
    given()
        .filter(validationFilter)
        .header("Authorization", "Bearer " + jwtToken)
        .when()
  ...
  @Test
  @Order(7)
  public void testUpdateEventContract() {
    // PUT /api/events/{id} should conform to OpenAPI spec
    given()
        .filter(validationFilter)
        .header("Authorization", "Bearer " + jwtToken)
        .contentType(ContentType.JSON)
        .body(
            "{\"name\":\"Updated Contract Test Event\",\"description\":\"Updated via contract"
                + " test\"}")
        .when()
  ...
  @Test
  @Order(8)
  public void testDeleteEventContract() {
    // DELETE /api/events/{id} should conform to OpenAPI spec
    given()
        .filter(validationFilter)
        .header("Authorization", "Bearer " + jwtToken)
        .when()
  ...
  @Test
  @Order(9)
  public void testGetNonExistentEventContract() {
    // GET /api/events/{id} for non-existent event should conform to OpenAPI spec (404)
    given().filter(validationFilter).when().get("/api/events/999999").then().statusCode(404);
  }

  @Test
  @Order(10)
  public void testUnauthorizedAccessContract() {
    // POST /api/events without auth should conform to OpenAPI spec (401)
    given()
        .filter(validationFilter)
        .contentType(ContentType.JSON)
        .body("{\"name\":\"Unauthorized Event\",\"description\":\"Should fail\"}")
        .when()
        .post("/api/events")
        .then()
        .statusCode(401);
