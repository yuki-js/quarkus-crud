...
public class LiveResource {
     ...
     * Clients can subscribe to this endpoint to receive live notifications
     * when rooms are created, updated, or deleted.
     * 
     * Example usage with curl:
     * curl -N http://localhost:8080/api/live/rooms
     * 
     * Example usage with JavaScript:
     * const eventSource = new EventSource('/api/live/rooms');
    ...
    public Multi<RoomEventResponse> streamRoomEvents() {
        return eventBroadcaster.getEventStream()
                .map(RoomEventResponse::from);
    }
}
